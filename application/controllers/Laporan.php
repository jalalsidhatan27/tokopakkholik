...
class Laporan extends CI_Controller
{
    ...
    public function index_owner()
    {
        ...
        if ($this->form_validation->run() == false) {
            template_view('laporan/index_owner', $data);
        } else {
            ...
            $this->cetak_pembelian($tgl1, $tgl2);
        }
    }

    public function cetak_pembelian($tgl1, $tgl2)
    {
        $this->load->library('Dompdf_gen');

        $data['tanggal'] = indo_date($tgl1) . " s/d " . indo_date($tgl2);
        $data['pembelian'] = $this->pembelian->getLaporanPembelian($tgl1, $tgl2);
        $data['jumlah'] = count((array) $data['pembelian']);
        $data['total'] = $this->pembelian->getTotalPembelian(null, [$tgl1, $tgl2]);
        ...
        ob_end_clean();
        $this->dompdf->stream("laporan_pembelian_" . time() . ".pdf", array('Attachment' => 0));
    }
